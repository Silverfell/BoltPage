...
2026-06-17 [code] Added [profile.release] strip=true + panic=abort (workspace root Cargo.toml); release binary 17.5MB->12.2MB: LINKEDIT 3.46->0.36MB (38526->247 syms), unwind tables 2.0->0.13MB. cargo build clean 55.96s.
2026-06-17 [note] Version bumped 2.2.1->2.2.2 (package.json source; sync-version.sh propagated to tauri.conf.json + Cargo.toml). Homebrew cask bumped at release per convention.
2026-06-24 [decision] Startup window resolution moved out of setup into RunEvent::Ready (resolve_startup_windows): tao/tauri pass macOS openURLs-vs-didFinishLaunching order through verbatim, so the setup-time welcome fallback raced RunEvent::Opened and left a stray empty window on Finder double-click.
...
2026-06-29 [note] CI clippy (newer than local 1.88) failed PR #33 on doc_lazy_continuation: the new STARTUP_OPENED_GRACE_MS const doc had absorbed resolve_startup_windows' bullet-list doc; reordered const above the fn doc with a blank line. Run `cargo clippy --all-targets -- -D warnings` locally before release, not just cargo check.
2026-06-29 [note] v2.2.4 released: dev->main via PR #33 (CI-gated), lightweight tag v2.2.4; release run 28402341154 published public Release (arm64/x64 DMG + x64 setup). Windows build unsigned this run (WINDOWS_CERTIFICATE not configured); macOS DMGs signed/notarized.
2026-06-29 [code] Homebrew cask bumped 2.2.3->2.2.4 (update-cask.sh, real sha256 both DMGs) and pushed to Silverfell/homebrew-tap; cask source committed to dev (0f07084).
