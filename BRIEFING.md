...
- Key decisions:
  ...
  - Release profile lives in the workspace root Cargo.toml (profiles are ignored in member crates): strip = true + panic = "abort". Drops the symbol table and unwind tables (binary 17.5MB to 12.2MB). panic=abort means any Rust panic aborts the process immediately instead of unwinding: no Drop runs on the panic path and a background-thread panic (notify watcher, tokio task) crashes the whole app rather than degrading. Officially recommended by Tauri; also removes the cross-FFI unwind UB that tao's catch_unwind guards. No app/core code uses catch_unwind; CI runs cargo test in the test profile (unwind) so it is unaffected.
  ...
  - Startup launch intent is enforced by a show-time gate, not timing (decision 126, replacing the 100ms-grace design): tao/tauri forward macOS openURLs-vs-didFinishLaunching order verbatim with no ordering contract, so any decide-by-deadline scheme stays racy. doc_open_requested and doc_open_paths are stored synchronously at event delivery (argv in setup via had_cli_args; the RunEvent::Opened match arm — never from a spawned task). resolve_startup_windows (RunEvent::Ready) restores the session or creates the welcome immediately, no grace timer; every window is created hidden (frontend shows it after first render via show_window), and resolver-created windows are tagged in startup_gated. show_window destroys a still-gated window unseen when a doc launch was observed — unless its decoded path is doc-requested, in which case it shows and becomes the requested document window (prevents same-label destroy/rebuild collisions, since file window labels are deterministic). Gate-destroy keeps session entries, so the saved session survives a file launch. The Opened handler retries a failed window build once (gate teardown may be in flight) and sweeps a still-pristine welcome; doc_open_succeeded guards a zero-windows reset of doc_open_requested if every requested open failed.
