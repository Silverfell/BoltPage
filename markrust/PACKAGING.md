# MarkRust Packaging and Distribution Guide

This document outlines the packaging and distribution process for MarkRust across Windows, macOS, and Linux platforms.
...
- **Output**: `target/release/bundle/msi/MarkRust_1.0.0_x64_en-US.msi`

#### NSIS Installer
- **Configuration**: Complete in `tauri.conf.json`
- **Features**: Custom installer UI, multiple language support
- **Build Command**: `cargo tauri build --target x86_64-pc-windows-msv`
- **Output**: `target/release/bundle/nsis/MarkRust_1.0.0_x64-setup.exe`
...
- **Output**: `target/release/bundle/dmg/MarkRust_1.0.0_x64.dmg`
...
xcrun notarytool submit "target/release/bundle/dmg/MarkRust_1.0.0_x64.dmg" \
  --apple-id "$APPLE_ID" \
  --password "$APPLE_PASSWORD" \
  --team-id "$APPLE_TEAM_ID" \
  --wait

xcrun stapler staple "target/release/bundle/dmg/MarkRust_1.0.0_x64.dmg"
```
...
- **Output**: `target/release/bundle/deb/markrust_1.0.0_amd64.deb`

#### AppImage (Universal Linux)
- **Configuration**: Complete in `tauri.conf.json`
- **Build Command**: `cargo tauri build --target x86_64-unknown-linux-gnu`
- **Output**: `target/release/bundle/appimage/MarkRust_1.0.0_amd64.AppImage`
...
Since MarkRust doesn't include auto-update functionality, users need to manually update:

1. **Windows**: Download and run new MSI/NSIS installer
2. **macOS**: Download new DMG and replace application
3. **Linux**: 
   - DEB: `sudo dpkg -i markrust_1.0.0_amd64.deb`
...
├── appimage/
│   └── MarkRust_1.0.0_amd64.AppImage
├── dmg/
│   └── MarkRust_1.0.0_x64.dmg
├── msi/
│   └── MarkRust_1.0.0_x64_en-US.msi
└── nsis/
    └── MarkRust_1.0.0_x64-setup.exe
