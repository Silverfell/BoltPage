...
- Run: `rm -rf /Applications/MarkRust.app && cp -r target/release/bundle/macos/MarkRust.app /Applications/`

### 3. CLEAN UP POLLUTION AFTER BUILD:
- Clean ALL Launch Services: `lsregister -kill -r -domain local -domain system -domain user`
- Register ONLY the Applications version: `lsregister -f /Applications/MarkRust.app`
- Verify single registration: `lsregister -dump | grep -c "path.*MarkRust.app"` must equal 1

### 4. VERIFICATION COMMANDS:
```bash
# Check registrations (must be exactly 1):
lsregister -dump | grep -c "path.*MarkRust.app"

# Test double-click works and loads content:
open test-file.md && sleep 3 && ps aux | grep markrust

# Check no launch-disabled flags:
lsregister -dump | grep -A20 MarkRust | grep "launch-disabled"
```
